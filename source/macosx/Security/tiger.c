 ...
 */
static void transform(tiger_ctx *hd, unsigned char *data)
{
...
static void tiger_init(tiger_ctx *hd)
{
...
static void tiger_update(tiger_ctx *hd, unsigned char *inbuf, size_t inlen)
{
...
static void tiger_final(unsigned char *hash, tiger_ctx *hd)
{
...
// public API
void Tiger ( const void* data, unsigned long length, char* outputBytes )
{
	tiger_ctx state;
	tiger_init(&state);
	tiger_update(&state, (unsigned char*)data, length);
	tiger_final((unsigned char*)outputBytes, &state);
}
